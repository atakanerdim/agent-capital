...
Ported from the desk this one was built beside, where it had already survived
four classes of failure. What changed here is the house rules, which describe a
trading simulation rather than a football desk, and "driven_by".

Usage:
  python kernel/runner.py --list --day tue
  python kernel/runner.py --agent desk --day tue
  python kernel/runner.py --agent momentum --day tue      (one advisor, for debugging)
  python kernel/runner.py --day tue            (every agent on duty, in roster order)
...
- The money here is imaginary. Every position, every trade and every result on this
  desk is a simulation. Never write as though real funds were at stake, and never
  address the reader as somebody whose own money you are advising. You are not.
- Nothing you write is financial advice, a recommendation, or a forecast anybody
  should act on. Say what you are doing and why you are doing it; do not tell a
  reader what to do.
- Real companies, currencies and commodities may be discussed only from public
  market data and the published record. Never invent earnings, news, deals,
  scandals, executive quotes, or anything else that reads as reported fact.
- No profanity, slurs, sexual content, threats, or harassment. The tone is a
  working desk talking to itself: dry, direct, occasionally funny, never abusive.
- No personal data about anyone. No politics, no religion.
- Never claim certainty about a price in the future. You may argue for a position;
  you may not promise an outcome.
...
def build_ctx(root, agent, day, mode, input_path):
    ...
    # The house rules travel in the context as well as glued to the prompt: a logic
    # module that drives other colleagues has to put them in front of those
    # colleagues too, and it must not have to reconstruct them by splitting a string.
    return {"house": HOUSE_RULES,
...
def main():
    ...
    # An advisor is on the floor every weekday, and the site says so, but its shift
    # is not its own to run: prices, eight decisions and the revaluation have to land
    # in one working tree or the day is half-written. Whoever carries them says so
    # with "driven_by", and the loop below leaves those colleagues to that agent.
    # Naming one explicitly with --agent still runs it, because debugging a single
    # advisor must stay possible.
    on_duty = [ag for ag in roster if day in ag["gunler"] and not ag.get("driven_by")]
    if a.list:
        for ag in on_duty:
            print(ag["id"])
        return
    picked = [ag for ag in roster if ag["id"] == a.agent] if a.agent else on_duty
    ok = True
    for ag in picked:
